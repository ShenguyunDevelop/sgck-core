...
public class CacheService<T> {
	 ...
	 * @param loader  对象自动加载类
	 * @param maximum 缓存的对象的最大数目
	 * @param expiredTimeInSec 缓存对象的过期时间，以秒为单位
	 */
	public CacheService(CacheItemLoader<T> loader,int maximum,int expiredTimeInSec){
		...
		this.cache = CacheBuilder.newBuilder()
		        .maximumSize(maximum)
		        .expireAfterAccess(expiredTimeInSec, TimeUnit.SECONDS)
		        .build(new CacheLoader<String, T>() {
                    @Override
                    public T load(String key) throws Exception {
                    	return itemLoader.load(key);
	...
	/**
	 * 该函数只是用于自动加载的cache(loadingCache)，
	 * 如果是非自动加载的缓存，请使用 {@code get(String key,Callable<T> caller)} 函数
	 * @param key 对象唯一标示
	 * @return
	 */
	...
	/**
	 * 如果是自动加载的cache(loadingCache)，会忽略caller参数
	 * 如果是非自动加载的缓存，必须指定caller对象
	 * @param key 对象唯一标示
	 * @return
	 */
